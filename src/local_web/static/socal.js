...
(() => {
 ...
 function sync(){
  ...
  for(const g of ['ladwp-region','sce-region','prime','commercial','gwp','gwp-demand'])$('socal-'+g).hidden=!(p?.input_groups||[]).includes(g);
  ...
  for(const o of f('solar_program').options)if(['none','approved_non_export'].includes(o.value))o.disabled=!!p?.solar_programs&&!p.solar_programs.includes(o.value);
 ...
 function request(){
  ...
  for(const n of ['eligibility_confirmed','ordinary_account_confirmed','cycle_confirmed','tax_confirmed','region_confirmed','interconnection_confirmed','heat_pump_water','non_cpp_confirmed','reactive_charge_exempt_confirmed','gwp_individual_meter_confirmed','gwp_allocation_confirmed','storage_schedule_confirmed','gwp_demand_history_confirmed'])a[n]=f(n).checked;
 ...
 function invalidate(){confirmationRevision++;savedResolution=null;for(const n of ['eligibility_confirmed','region_confirmed','tax_confirmed','interconnection_confirmed','ordinary_account_confirmed','non_cpp_confirmed','reactive_charge_exempt_confirmed','gwp_individual_meter_confirmed','gwp_allocation_confirmed','storage_schedule_confirmed','gwp_demand_history_confirmed'])f(n).checked=false;$('socal-resolution').textContent='Location or account changed. Confirm again.';}
 ...
 form.addEventListener('change',e=>{if(['latitude','longitude','utility','site_type','site_subtype','sc_mode','sc_generation','sc_reference','sc_delivery_confirmed'].includes(e.target.name))invalidate();if(['start_date','end_date','sc_tariff_id','pv_choice','sc_solar_program'].includes(e.target.name)){f('eligibility_confirmed').checked=false;f('storage_schedule_confirmed').checked=false;f('interconnection_confirmed').checked=false;}sync();});
