...
def validate_study(request):
    if not isinstance(request,dict) or set(request)!=FIELDS or request.get('schema_version')!=4:
    ...
    if not isinstance(request['battery'],dict) or set(request['battery'])!={'capacity_kWh','energy_kWh','max_charge_kw','max_discharge_kw','SOC_min','SOC_max','charge_efficiency','discharge_efficiency'}:
...
def execute_study(directory,request):
    ...
    write_json(directory/'result.json',dict(schema_version=4,tables=tables,warnings=warnings,
        ...
        tariff=result['bill']['tariff'],input_provenance={'resolution':request['resolution'],'account':request['account']}))
