...
def optimize(key,inputs,account,start,end,battery,wear,*,include_degradation_in_optimization=False):
 ...
 bill_expression=sum(lines.values())
 problem=cp.Problem(cp.Minimize(bill_expression+(degradation if include_degradation_in_optimization else 0)),constraints)
 ...
 if not include_degradation_in_optimization:
  # A bill-only optimum can have many equally priced charge/discharge paths.
  # Select the lowest-throughput path among them without pricing wear in the bill.
  tie_tolerance=max(1e-5,abs(optimal_objective)*1e-8)
  tie_problem=cp.Problem(cp.Minimize(cp.sum(charge+discharge)),constraints+[bill_expression<=optimal_objective+tie_tolerance])
  tie_problem.solve(solver='SCIPY' if tie_problem.is_mixed_integer() else 'CLARABEL')
  if tie_problem.status!='optimal':raise ValueError('Bill-optimal dispatch tie-break did not converge: '+str(tie_problem.status))
 ...
 gap=abs(authoritative['total']+(degradation if include_degradation_in_optimization else 0)-optimal_objective)
 if gap>max(.02,abs(optimal_objective)*1e-6):raise ValueError(f'Bill and optimized objective do not reconcile: bill={authoritative["total"]:.6f}, objective={optimal_objective:.6f}, gap={gap:.6f}.')
